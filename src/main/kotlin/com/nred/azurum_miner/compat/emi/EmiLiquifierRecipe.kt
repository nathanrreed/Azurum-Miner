...
class EmiLiquifierRecipe(id: ResourceLocation, inputs: List<EmiIngredient>, output: EmiStack, val power: Int, val processingTime: Int) : BasicEmiRecipe(LIQUIFIER_CATEGORY, id, 100, 69) {
    ...
    override fun addWidgets(widgets: WidgetHolder) {
        ...
        widgets.addDrawable(width - 33, 2, 31, 65) { guiGraphics, _, _, _ -> guiGraphics.blitSprite(Companion.TANK, 0, 0, 150, 31, 65) }

        widgets.addDrawable(2, 2, 6, 65) { guiGraphics, _, _, _ -> guiGraphics.blitSprite(Companion.ENERGY_BAR, 0, 0, 3, 6, 65) }.tooltipText(listOf(Component.literal(getFE(power))))
        widgets.addTexture(Companion.ENERGY_INNER.withPath("textures/gui/sprites/common/energy_inner.png"), 3, 3, 4, 63, 0, 0, 4, 63, 4, 2)
    }
}