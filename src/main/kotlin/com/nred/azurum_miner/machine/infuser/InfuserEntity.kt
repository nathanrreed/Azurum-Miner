    ...
    fun tick(level: Level, pos: BlockPos, state: BlockState, blockEntity: BlockEntity) {
        ...
        for (recipe in level.recipeManager.getRecipesFor(ModRecipe.INFUSER_RECIPE_TYPE.get(), InfuserInput(state, itemStackHandler.getStackInSlot(0), itemStackHandler.getStackInSlot(1), fluidHandler.getFluidInTank(0)), level).map { it.value }) {
            if (recipe != null) {
                ...
                if (energyHandler.energyStored >= power && data[IS_ON] == TRUE && !itemStackHandler.getStackInSlot(0).isEmpty && this.fluidHandler.internalExtractFluid(recipe.inputFluid, IFluidHandler.FluidAction.SIMULATE).amount == recipe.inputFluid.amount && this.itemStackHandler.internalInsertItem(2, recipe.result, true).isEmpty) {
                    found = true
