...
class LiquifierScreen(menu: LiquifierMenu, playerInventory: Inventory, title: Component) : AbstractContainerScreen<LiquifierMenu>(menu, playerInventory, title) {
    ...
    fun resize(width: Int, imageWidth: Int, height: Int, imageHeight: Int) {
        ...
        tank = ScreenRectangle(base.right() - 65, base.top() + 5, 34, 77)
    ...
    override fun renderBg(guiGraphics: GuiGraphics, partialTick: Float, mouseX: Int, mouseY: Int) {
        ...
        guiGraphics.blitSprite(SLOT, base.left() + LiquifierMenu.slot_x, base.top() + LiquifierMenu.slot_y, 18, 18)
        ...
        if (!menu.fluidHandler!!.getFluidInTank(0).isEmpty) {
            val varMoltenLen = floor(menu.fluidHandler!!.getFluidAmount(0).toDouble() / LiquifierEntity.FLUID_SIZE.toDouble() * (tank.height - 2)).toInt()
        ...
        if (tank.containsPoint(mouseX, mouseY)) {
            guiGraphics.renderTooltip(font, Component.translatable("tooltip.azurum_miner.liquifier.info.tank", menu.fluidHandler!!.getFluidInTank(0).fluidType.description, DecimalFormat("#,###").format(menu.fluidHandler!!.getFluidAmount(0))), mouseX, mouseY)
        }
    }
