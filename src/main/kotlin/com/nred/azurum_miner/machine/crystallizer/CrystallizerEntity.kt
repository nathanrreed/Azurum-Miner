...
open class CrystallizerEntity(pos: BlockPos, blockState: BlockState) : AbstractMachineBlockEntity(ModBlockEntities.CRYSTALLIZER_ENTITY.get(), "crystallizer", pos, blockState), IMenuProviderExtension {
    ...
    fun tick(level: Level, pos: BlockPos, state: BlockState, blockEntity: BlockEntity) {
        ...
        if (recipe != null) {
            ...
            if (energyHandler.energyStored >= power && data[IS_ON] == TRUE && !itemStackHandler.getStackInSlot(0).isEmpty && this.fluidHandler.internalExtractFluid(recipe.inputFluid, IFluidHandler.FluidAction.SIMULATE).amount == recipe.inputFluid.amount && this.itemStackHandler.internalInsertItem(1, recipe.result, true).isEmpty) {
