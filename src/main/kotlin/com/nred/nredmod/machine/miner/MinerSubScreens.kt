...
import com.nred.nredmod.machine.miner.MinerEntity.Companion.get
import com.nred.nredmod.screen.GuiCommon.Companion.getFE
import com.nred.nredmod.screen.GuiCommon.Companion.getTime
import com.nred.nredmod.screen.RenderTab
import com.nred.nredmod.util.Helpers.compC
import com.nred.nredmod.util.Helpers.compCat
...
class MainTab(val menu: MinerMenu) : RenderTab(TITLE) {
    ...
    companion object {
        private val TITLE: Component = Component.translatable("tab." + NredMod.ID + ".miner.main.title")
        val POINTS_BUTTON = ResourceLocation.fromNamespaceAndPath(NredMod.ID, "common/points_button")
        val POINTS_BUTTON_DISABLED = ResourceLocation.fromNamespaceAndPath(NredMod.ID, "common/points_button_disabled")
        val POINTS_BUTTON_HOVER = ResourceLocation.fromNamespaceAndPath(NredMod.ID, "common/points_button_hover")
        val POINTS_BUTTON_BARS = ResourceLocation.fromNamespaceAndPath(NredMod.ID, "common/points_button_bars")
            ...
            val imageBtn = object : ImageButton(0, 0, 24, 54, buttonSprites, { }) {
                ...
                fun modifierToolTip(): Tooltip {
                    ...
                    if (points < 5) {
                        ...
                        val lines = arrayOf(
                            compC("tooltip.nredmod.miner.button${num + 1}.upgrade${points + 1}", if (data[TOTAL_MODIFIER_POINTS] - data[USED_MODIFIER_POINTS] < 1) CommonColors.GRAY else CommonColors.GREEN, vals[2], vals[3], vals[0]), // Upside
                            compC("tooltip.nredmod.miner.button.FEChange", CommonColors.SOFT_RED, getFE(data[ENERGY_NEEDED]), vals[1]) // Power use
                        )

                        return Tooltip.create(compCat(compC("tooltip.nredmod.miner.button${num + 1}"), *lines))
                    }
                    return Tooltip.create(compCat(compC("tooltip.nredmod.miner.button${num + 1}")))
                }
                ...
                override fun renderWidget(guiGraphics: GuiGraphics, mouseX: Int, mouseY: Int, partialTick: Float) {
                    ...
                    if (this.active) {
                        if (Screen.hasShiftDown()) {
                            this.tooltip = Tooltip.create(Component.translatable("tooltip.nredmod.miner.button${num + 1}.extended"))
                        } else {
                            this.tooltip = modifierToolTip()// Component.translatable("tooltip.nredmod.miner.button${num + 1}"))
                        }
                    } else {
                        this.tooltip = Tooltip.create(Component.translatable("tooltip.nredmod.miner.button.disabled", Component.literal("${num + 1}").setStyle(Style.EMPTY.withBold(true)), Component.translatable("tooltip.nredmod.miner.button.${num + 1}").withColor((0xFF888888).toInt())))
                    }
                }
            }
...
//        val lines = listOf("Acc: ${data.get(MinerEntity.Companion.MinerEnum.ACCURACY.ordinal)}%", "Raw: ${data.get(MinerEntity.Companion.MinerEnum.RAW_CHANCE.ordinal)}%")
        ...
        for ((idx, line) in lines.withIndex()) {
            ...
            if (rect.containsPoint(mouseX, mouseY)) {
                guiGraphics.renderTooltip(font, Component.translatable("tooltip.nredmod.miner.info$idx"), mouseX, mouseY)
            }
...
class OptionsTab(val menu: MinerMenu) : RenderTab(TITLE) {
    companion object {
        private val TITLE: Component = Component.translatable("tab." + NredMod.ID + ".miner.options.title")
    }
