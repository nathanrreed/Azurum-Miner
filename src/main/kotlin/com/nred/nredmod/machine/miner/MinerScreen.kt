...
import com.nred.nredmod.screen.SpriteTabButton
import com.nred.nredmod.screen.VerticalTabNavigationBar
import com.nred.nredmod.util.Helpers.componentSplit
...
class MinerScreen(menu: MinerMenu, playerInventory: Inventory, title: Component) : AbstractContainerScreen<MinerMenu>(menu, playerInventory, title) {
    companion object {
        ...
        val TANK = ResourceLocation.fromNamespaceAndPath(NredMod.ID, "miner/fluid_tank")
    }
    ...
    override fun renderBg(guiGraphics: GuiGraphics, partialTick: Float, mouseX: Int, mouseY: Int) {
        ...
        guiGraphics.blitSprite(ResourceLocation.fromNamespaceAndPath(NredMod.ID, "miner/bar"), barRects[0].left(), barRects[0].top(), barGraphs.width, barRects[0].height)
        ...
        guiGraphics.blitSprite(ResourceLocation.fromNamespaceAndPath(NredMod.ID, "miner/bar"), barRects[1].left(), barRects[1].top(), barGraphs.width, barRects[1].height)
        ...
        guiGraphics.blitSprite(ResourceLocation.fromNamespaceAndPath(NredMod.ID, "miner/bar"), barRects[2].left(), barRects[2].top(), barGraphs.width, barRects[2].height)
        ...
        guiGraphics.blitSprite(ResourceLocation.fromNamespaceAndPath(NredMod.ID, "miner/bar"), barRects[3].left(), barRects[3].top(), barGraphs.width, barRects[3].height)
        ...
        guiGraphics.blitSprite(ResourceLocation.fromNamespaceAndPath(NredMod.ID, "miner/molten_ore_still"), tank.left(), tank.bottom() - varMoltenLen, tank.width, varMoltenLen)
        ...
        if (powerButton.containsPoint(mouseX, mouseY)) {
            guiGraphics.renderTooltip(font, Component.translatable("tooltip.nredmod.machine." + if (menu.containerData[IS_ON] == 1) "on" else "off"), mouseX, mouseY)
        }
        if (tank.containsPoint(mouseX, mouseY)) {
            guiGraphics.renderTooltip(font, componentSplit("tooltip.nredmod.miner.info.tank", listOf(Style.EMPTY, Style.EMPTY.withColor(0xAAAAAA)), DecimalFormat("#,###").format(menu.containerData[MOLTEN_ORE_LEVEL])), mouseX, mouseY)
        }
        for ((idx, bar) in barRects.withIndex()) {
            if (bar.containsPoint(mouseX, mouseY)) {
                guiGraphics.renderTooltip(font, Component.translatable("tooltip.nredmod.miner.chance${idx}", String.format("%.1f", barData[idx] * 100)), mouseX, mouseY)
            }
        }
    }
